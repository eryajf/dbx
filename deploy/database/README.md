...
This directory contains reproducible Docker Compose environments for manual database verification. Each versioned recipe uses a pinned image, a named volume, a `0.0.0.0` port binding, a health check, and initialization data or smoke data created during verification.
...
Every network recipe uses the standard database port plus one on the host. The default password is `123456`, and the default database is `dbx`; Redis uses DB 0 and the `dbx:` key prefix. Container names follow `dbx-<product>-<version>`. Versions of the same database share the default host port, so use `DB_PORT` when running them concurrently. `DB_PORT` and `DB_PASSWORD` override the default host port and password. Because services listen on all host interfaces, set a strong `DB_PASSWORD` and protect the host with firewall rules on shared networks. `make db-reset` deletes the named volume and therefore requires `CONFIRM=1`:
