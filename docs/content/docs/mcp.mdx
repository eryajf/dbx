...
Updated servers ignore `DBX_MCP_ALLOW_WRITES` and `DBX_MCP_ALLOW_DANGEROUS_SQL`. Legacy connection-scope variables are read only for compatibility and can only narrow the DBX allowlist.
