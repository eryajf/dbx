...
<Tabs groupId="platform" items={['macOS', 'Windows', 'Linux', 'Docker']}>
  ...
  <Tab value="Docker">
    ...
    建议固定使用明确的发布版本标签而非 `latest`，以保证部署可复现。`./data` 会将 DBX 数据保存在当前目录下；中国大陆用户可改用 `docker.cnb.cool/dbxio.com/dbx:0.5.52`，以获得更快的拉取速度。
    ...
    在 Docker Compose 中，`./data` 相对于该 Compose 文件所在目录。
...
| Docker / Web | 服务器自托管、浏览器访问                               | HTTP 路由调用同一套 Rust core | 宿主机上的 `data` 目录        |
