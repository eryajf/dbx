...
数据库测试实验室提供轻量、固定版本的 Docker Compose 环境，方便使用真实数据库版本验证 DBX。每个配方都使用命名数据卷、健康检查、初始化数据或验证阶段创建的冒烟数据，并且默认监听 `0.0.0.0`。

<Callout type="warning">默认密码为 `123456`，端口会监听宿主机的全部网络接口。在共享网络中使用前请设置 `DB_PASSWORD`，并配置常规防火墙规则。这些环境用于开发，不是生产部署模板。</Callout>
...
`DB_PORT` 修改主机侧端口，`DB_PASSWORD` 修改默认密码。例如：
...
`recipe.json` 提供 DBX 连接字段和冒烟命令数组。不要使用 shell 字符串，命令必须作为参数数组执行。Compose 文件必须固定镜像版本、使用命名卷和健康检查、将端口绑定至 `0.0.0.0`，并把 `container_name` 设置为 `dbx-<product>-<version>`。
