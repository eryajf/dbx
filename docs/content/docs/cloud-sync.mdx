...
DBX stores an encrypted `dbx-sync.json` file in the Gist. New uploads and encrypted GitHub/Gitee snippets require the sync password, so the snippet URL does not expose connection details, SQL, or settings. Existing plain-text snippets can still be downloaded once for migration.

<Callout type="warn">
Plain-text snippets created by older versions cannot be safely migrated in place: updating the same Gist does not remove its revision history. Use **Migrate legacy snippet** instead. DBX encrypts the existing remote snapshot into a new snippet first, then deletes the old plaintext snippet only after the new ID is stored locally. If the old snapshot contains encrypted secrets, enter its original sync password so DBX can verify them before deleting the old copy. If deletion fails, DBX preserves the new ID and tells you which old snippet must be deleted manually. Rotate any credentials the old snippet may have exposed.
</Callout>
...
GitHub Gist and Gitee snippets always encrypt the entire snapshot with the sync password. The password is never uploaded and cannot be recovered.
...
<Callout type="warn">
The sync passphrase cannot be recovered. An encrypted GitHub/Gitee snippet cannot be restored without it. For WebDAV, a snapshot without encrypted secrets can still be restored without a passphrase.
</Callout>
