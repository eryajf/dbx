...
DBX 会在 Gist 中保存一个经过同步密码加密的 `dbx-sync.json` 文件。新的上传和已加密的 GitHub/Gitee 代码片段都必须提供同步密码，片段链接本身不会暴露连接信息、SQL 或设置；历史明文片段仍可下载一次以便迁移。

<Callout type="warn">
旧版本创建的明文片段不能安全地原地迁移：更新同一个 Gist 不会删除历史修订。请使用 **迁移旧版代码片段** 操作。DBX 会先将已有的远端快照加密写入新片段，并在新 ID 持久化到本地后再删除旧明文片段；若旧快照包含已加密的敏感信息，请输入其原同步密码，让 DBX 在删除旧副本前完成验证。如果删除失败，DBX 会保留新 ID 并提示需要手动删除的旧片段。请轮换旧片段中可能已经暴露的凭据。
</Callout>
...
GitHub Gist 和 Gitee 代码片段始终使用同步密码加密整个快照。密码不会上传，也无法找回。
...
<Callout type="warn">
同步密码无法找回。GitHub/Gitee 的加密代码片段没有同步密码就无法恢复；WebDAV 中不含加密敏感信息的快照仍可不使用同步密码恢复。
</Callout>
