...
The service ports listen on `0.0.0.0`; set `DB_PASSWORD` and use firewall controls before exposing a host to a shared network. See the [Database Test Lab](/en/docs/database-lab) for all supported recipes, overrides, and reset safety requirements.
