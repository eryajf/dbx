...
When testing the MCP bridge, the instance started last owns the shared `mcp-bridge-port` discovery file and receives MCP requests. Redis PubSub uses port `4224` by default; use `DBX_PORT` to assign the development instance a different port when testing it alongside DBX.
