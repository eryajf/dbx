...
Qdrant uses `123456` as its administrator API key, passed as the `api-key` HTTP header rather than a username/password pair. Its HTTP and gRPC ports default to `6334` and `6335`. To use the ports in the upstream Docker command, run:
