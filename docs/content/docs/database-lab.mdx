...
The Database Test Lab provides small, pinned Docker Compose environments for testing DBX against real database versions. Every recipe uses a named data volume, a health check, initialization data or smoke data created during verification, and listens on `0.0.0.0` by default.

<Callout type="warning">The default password is `123456` and ports listen on all host interfaces. Change `DB_PASSWORD` before using a shared network, and apply your normal firewall controls. These environments are for development, not production deployment.</Callout>
...
`DB_PORT` changes the host-side port and `DB_PASSWORD` changes the default password. For example:
...
`recipe.json` supplies the DBX connection fields and smoke command array. Do not use a shell string for smoke checks: commands execute as argument arrays. Compose files must pin an image, use a named volume and health check, bind ports to `0.0.0.0`, and set `container_name` to `dbx-<product>-<version>`.
