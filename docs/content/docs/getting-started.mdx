...
<Tabs groupId="platform" items={['macOS', 'Windows', 'Linux', 'Docker']}>
  ...
  <Tab value="Docker">
    ...
    Pin a release tag instead of `latest` so deployments remain reproducible. The `./data` directory stores DBX data beside your current directory. For faster pulls in China, use `docker.cnb.cool/dbxio.com/dbx:0.5.52` instead.
    ...
    In Docker Compose, `./data` is relative to the directory that contains the Compose file.
...
| Docker / Web | Self-hosted access from a server or browser                           | HTTP routes calling the same Rust core | `data` directory on the host           |
