...
Choose **Read only**, **Data read/write**, or **Full access** in DBX instead of placing permission flags in client configuration. Updated servers ignore `DBX_MCP_ALLOW_WRITES` and `DBX_MCP_ALLOW_DANGEROUS_SQL` so clients cannot widen the DBX policy.
...
请在 DBX 中选择 **只读**、**数据读写** 或 **完全访问**，不要在客户端配置中放置权限开关。新版 Server 忽略 `DBX_MCP_ALLOW_WRITES` 和 `DBX_MCP_ALLOW_DANGEROUS_SQL`，客户端不能放宽 DBX 中央策略。
