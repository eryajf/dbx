...
test("Tauri AI agent stream events carry their session id", () => {
  ...
  assert.match(tauriAiCommandSource, /(?:let event_session_id = session_id\.clone\(\);|session_id: self\.session_id\.clone\(\))/);
  assert.match(tauriAiCommandSource, /AiAgentEventPayload \{ session_id: (?:event_session_id|self\.session_id)\.clone\(\), event \}/);
