...
import { clampConnectionSqlSafety, connectionReadOnlyReason } from "./mcp-policy.js";
import { supportsHashLineComments } from "./sql-risk.js";
import { chainedMethodCallPattern, describeMongoCommandParseFailure, findChainedMethodCallIndex, findMatchingParen, normalizeJsonArgument, parseCollectionMethodTarget, parseMongoAggregateCommand, splitTopLevel, type MongoAggregateCommand } from "@dbx-app/mongo-shell";

export { describeMongoCommandParseFailure, parseMongoAggregateCommand, MONGO_SHELL_COMMAND_HINT } from "@dbx-app/mongo-shell";
...
async function executeRedisCommandDirect(config: ConnectionConfig, db: number, commandText: string, options?: RedisCommandOptions): Promise<RedisCommandResult> {
  ...
  if (!options?.skipSafetyCheck && safety === "blocked") {
    throw new Error("Redis command is blocked by the current DBX MCP high-risk operation policy.");
  }
...
export function evaluateMongoWriteSafety(command: MongoWriteCommand, options: { allowWrites?: boolean; allowDangerous?: boolean }): { allowed: boolean; reason?: string } {
  ...
  const highRisk = command.kind === "update" || command.kind === "delete" ? mongoFilterIsEffectivelyUnbounded(command.filter) : command.kind !== "insert";
...
function mongoFilterObjectIsUnbounded(filter: Record<string, unknown>): boolean {
  ...
  return entries.every(([key, value]) => {
    ...
    if (key === "$and") {
      return !Array.isArray(value) || value.length === 0 || value.every((clause) => !isRecord(clause) || mongoFilterObjectIsUnbounded(clause));
    }
    if (key === "$or") {
