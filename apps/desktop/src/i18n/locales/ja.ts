...
export default withEnglishFallback({
  ...
  connection: {
    ...
    mcpAccess: "MCPアクセス",
    mcpAccessHint: "DBXが管理するこのポリシーが最終的な制約です。クライアント環境変数はMCPセッションをさらに制限することだけができます。",
    mcpAccessDisabled: "無効",
    mcpAccessReadOnly: "読み取り専用",
    mcpAccessReadWrite: "読み書き",
  ...
  settings: {
    ...
    mcpScopeConnection: "セッションの接続範囲",
    mcpScopeConnectionDescription: "生成するMCP設定を選択した複数のDBX接続に限定できます。この設定はアクセス範囲を狭めることだけができます。",
    mcpScopeAllConnections: "MCPが有効なすべての接続",
    mcpScopeConnectionUnavailable: "以前選択した接続（利用不可）",
    mcpScopeConnectionUnavailableDescription: "選択済みの{count}件の接続が存在しないかMCPが無効です。解除するまでIDはscopeに残ります。",
