...
export default withEnglishFallback({
  ...
  connection: {
    ...
    mcpAccess: "MCP 访问权限",
    mcpAccessHint: "此策略由 DBX 管理并作为最终约束；客户端环境变量只能进一步收紧 MCP 会话权限。",
    mcpAccessDisabled: "禁用",
  ...
  settings: {
    ...
    mcpScopeConnection: "会话连接范围",
    mcpScopeConnectionDescription: "可将生成的 MCP 配置限制到多个指定的 DBX 连接。此选项只能收窄范围，不能覆盖连接自身的 MCP 权限。",
    mcpScopeAllConnections: "所有已启用 MCP 的连接",
    mcpScopeConnectionUnavailable: "之前选择的连接（不可用）",
    mcpScopeConnectionUnavailableDescription: "有 {count} 个已选连接不存在或已禁用 MCP；取消勾选前，其 ID 仍会保留在 scope 中。",
    ...
    mcpReadonlyModeDescription: "开启后会为示例配置附加 DBX_MCP_ALLOW_WRITES=0。它可以收紧会话权限，但不能覆盖 DBX 管理的连接策略。",
